...
static int free_dind(int dev, int block)
{
	...
	// 首先判断参数的有效性。如果逻辑块号为0,则返回。然后读取二次间接块的一级块，并释放其上
	// 表明使用的所有逻辑块，然后释放该一级块的缓冲块。
	if (!block)
		return 1;
	block_busy = 0;
	if (bh = bread(dev, block)) {
...
void truncate(struct m_inode * inode)
{
	...
	if (!(S_ISREG(inode->i_mode) || S_ISDIR(inode->i_mode) ||
		...
		return;
	// 然后释放i节点的7个直接逻辑块，并将这7个逻辑块项全置零。函数free_block()用于释放设备
	// 上指定逻辑块的磁盘块(fs/bitmap.c)。若有逻辑块忙而没有被释放则置块忙标志block_busy。
repeat:
	block_busy = 0;
	for (i = 0; i < 7; i++)
	...
	// 最后重新置文件修改时间和i节点改变时间为当前时间。宏CURRENT_TIME定义在头文件
	// include/linux/sched.h中，定义为（startup_time+jiffies/HZ）。用于取得从1970:0:0:0
	// 开始到现在为止经过的秒数。
