 ...
 */
/*
 * OK，由于我没有得到任何其他有关出错号的资料，我只能使用与 minix 系统相同的出
 * 错号了。希望这些是POSIX兼容的或者在一定程度上是这样的,我不知道(而且POSIX没
 * 有告诉我 - 要获得他们的标准需要出钱)。
 *
...
// 或者其他一些特定的值来表示。但是这个返回值仅说明错误发生了。如果需要知道出错的
// 类型，就需要查看表示系统出错号的变量 errno 。该变量即在 errno.h 文件中声明。
// 在程序开始执行时该变量值被初始化为 0。

extern int errno;

// 在出错时，系统调用会把出错号放在变量 errno 中(负值)，然后返回-1。因此程序若需要
// 知道具体错误号，就需要查看 errno 的值。
