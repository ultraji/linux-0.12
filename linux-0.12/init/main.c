...
// CMOS 的访问时间很慢。为了减小时间误差，在读取了下面循环中所有数值后，若此时CMOS中秒值发生了
// 变化，则重新读取。这样能控制误差在1s内。
...
// init()函数运行在任务0第一次创建的子程序(任务1)中。首先对第一个将要执行的程序(shell)的环
// 境进行初始化，然后以登录shell方式加载该程序并执行。
void init(void)
{
	int pid,i;
...
// 下面以读写方式打开“/dev/tty1”,它对应终端控制台。因为这是第一次打开文件操作，所以产生的文件
// 句柄号(文件描述符)肯定是 0。
