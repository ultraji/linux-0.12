...
// 物理内存管理初始化
//
// 该函数对 1MB 以上内存区域以页面为单位进行管理前的初始化设置工作。一个页面长度为 4KB 。该函
// 数 1MB 以上所有物理内存划分成一个个页面，并使用一个页面映射字节数组mem_map[]来管理所有这些页
// 面。对于具有 16MB 内存容量的机器，该数组共有3840项((16MB-1MB)/4KB)，即可管理3840个物理页面。
// 每当一个物理内存页面被占用时就把 mem_map[] 中对应的字节值增1；若释放一个物理页面，就把对应字
// 节值减 1。若字节值为0，则表示对应页面空闲；若字节值大于或等于1，则表示对应页面被占用或被不同程
// 序共享占用。在该版本的 Linux 内核中，最多能管理16MB的物理内存，大于16MB的内存将弃置不用。对于
// 具有 16MB 内存的 PC 系统，在没有设置虚拟盘 RAMDISK 的情况下,共有3072个物理页面可供分配。而范围
// 0~1MB 内存空间用于内核系统(其实内核只使用 0~640KB,剩下的部分被部分高速缓冲和设备内存占用)。参数
// start_mem 是可用作页面分配的主内存区起始地址(已去除RAMDISK所占内存空间)。end_mem 是实际物理内
// 存最大地址。而地址范围 start_mem 到 end_mem 是主内存区。
...
void mem_init(long start_mem, long end_mem)
{
	...
	// 然后计算主内存区起始内存 start_mem 处页面对应内存映射字节数组中项号i和主内存区页面数。此时
	// mem_map[] 数组的第 i 项正对应主内存区中第 1 个页面。最后将主内存区中页面对应的数组项清零
	// (表示空闲)。对于具有16MB物理内存的系统，mem_map[]中对应4MB~16MB主内存区的项被清零。
